function irradiatePlan_Arduino(Fcfg, COMstage, COMtrigger, thePlan, vector2startPoint, stageLimits, airDepthAtPos0, app)
...
elseif strcmp(thePlan.mode, 'TRIG')
    fprintf(logFile, '\tArduino high time in us: %s\n', thePlan.widthT_us);
end
...
for i=1:numel(thePlan.X)
    ...
    elseif strcmp(thePlan.mode, 'TRIG')
        ...
        msg = sprintf('Sending trigger signal for %u shots with %3.3f us distance', thePlan.Nshots(i), thePlan.T_out_us(i));  
        fprintf(logFile, [datestr(now,'[HH:MM:SS.FFF] ') msg '\n']);

        msg = runArduinoTrigger(COMtrigger,  thePlan.Nshots(i), thePlan.widthT_us, thePlan.T_out_us(i));      
        fprintf(logFile, [datestr(now,'[HH:MM:SS.FFF] ') msg '\n']);    
    else
