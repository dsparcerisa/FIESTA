...
save('contours_feb19_Au.mat');

%% Final values
clear all
load('contours_feb19_Au.mat');

meanIntegralDose = mean(integralDose, 2, 'omitnan');
errorIntegralDose = std(integralDose, [], 2, 'omitnan');

%sigmasX_mm(6,3:4) = nan
%deltasigmasX_mm(6,3:4) = nan
...
errorSigmaX(6) = sqrt(mean(deltasigmasX_mm(6,:),2,'omitnan').^2 + (std(sigmasX_mm(6,:),[],2, 'omitnan')/sqrt(N_ITERATIONS)).^2)
