...
filePaths = {fullfile(Fcfg.filmPath,'Rad14a-Cal0001.tif'),fullfile(Fcfg.filmPath,'Rad14b-Cal0001.tif'),fullfile(Fcfg.filmPath,'Rad14c-Cal0001.tif')};
nCrops = [2 2 1];
...
save('contours_feb6_R14.mat');

%% Buscar centros y sigmas --> Realinear y recropear automáticamente para que ambas sean idénticas

clear all
load('contours_feb6_R14.mat');

fac = 5; % how many times HD is supposed to be larger than LD
...
subplot(1,2,2)
yyaxis left
errorbar(zOrdered, meanDoses(izO), errorDoses(izO),'bo:'); hold on
ylabel('Central dose (Gy)');
yyaxis right
errorbar(zOrdered, meanIntegralDose(izO), errorIntegralDose(izO), 'rx:');
ylim([0 10e4]);
ylabel('Intregral dose (a.u.)');
xlabel('Air distance (cm)');
title('Dose');
grid on
