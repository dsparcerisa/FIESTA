...
Nfiles = 1;
filePaths = {fullfile(Fcfg.filmPath,'Rad-Cu0001.tif')};
nCrops = 6;
...
save('contours_feb19_Cu.mat');

%% Final values
clear all
load('contours_feb19_Cu.mat');

meanIntegralDose = mean(integralDose, 2, 'omitnan');
errorIntegralDose = std(integralDose, [], 2, 'omitnan');
meanSigmaX = mean(sigmasX_mm, 2)
meanSigmaY = mean(sigmasY_mm, 2)
meanDoses = mean(doseCentralValue, 2)

errorSigmaX = sqrt(mean(deltasigmasX_mm,2).^2 + (std(sigmasX_mm,[],2)/sqrt(N_ITERATIONS)).^2)
errorSigmaY = sqrt(mean(deltasigmasY_mm,2).^2 + (std(sigmasY_mm,[],2)/sqrt(N_ITERATIONS)).^2)
...
F2x = fit(zValues', meanSigmaX, 'poly2', 'Weight', 1./errorSigmaX.^2)
F2y = fit(zValues', meanSigmaY, 'poly2', 'Weight', 1./errorSigmaY.^2)
