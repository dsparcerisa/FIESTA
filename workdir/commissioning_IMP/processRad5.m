...
fullPath = [basePath filesep 'R5.tif'];
I  = imread(fullPath);
I = uint16(I);
...
%% Correction for dose in well
% TODO!

%% Correction for relative efficiency
% TODO!

%% Plot
figure
circleMask = dose.data>1;
subplot(1,3,1);
dose.plotSlice
hold on
contour(dose.getAxisValues('X'), dose.getAxisValues('Y'), circleMask', 1, 'w', 'LineWidth', 3)
axis([-0.3 0.3 -0.4 0.2])
caxis([1 20])
colorbar
subplot(1,3,2);
[C,h] = contour(dose.getAxisValues('X'), dose.getAxisValues('Y'), dg', 1:3:22);
grid on
xlabel('X'); ylabel('Y');
caxis([1 20])
clabel(C,h);
axis([-0.3 0.3 -0.4 0.2])
subplot(1,3,3);
histogram(dose.data(circleMask==1),'Normalization','probability');
xlabel('Dose (Gy)');
title('Dose points within well');
grid on
mean(dose.data(circleMask==1))
std(dose.data(circleMask==1))