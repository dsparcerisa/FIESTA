...
fullPath = [basePath filesep 'R2.tif'];
I  = imread(fullPath);
I = uint16(I);
...
for i=1:Npoints
    ...
    %negValuesMask = dose.data<0 | dose.data>12;
    %dose.data(negValuesMask) = dg(negValuesMask);
...
colorbar
subplot(1,2,2);
[C,h] = contour(dose.getAxisValues('X'), dose.getAxisValues('Y'), dg', [0.5 1 3 5 10 30]);
xlabel('X'); ylabel('Y');
caxis([1 40])
set(gca,'ColorScale','log')
clabel(C,h);
