...
fullPath = [basePath filesep 'EBT3unl' filesep '3MeV'];
radName = 'Rad1';
...
logName = 'irrLog_2020_07_08_14_08_03.log';
...
% Corregir el error de la pimera (solo para Rad1 y Rad2)
Zpos2 = Zpos;
Zpos(5) = Zpos(3);
...
%% Predicción de la dosis central
theMask=1:9;
finalSigmasX = 0.5*(sigmaX_mm(1:9) + sigmaX_mm(12:20));
finalSigmasY = 0.5*(sigmaY_mm(1:9) + sigmaY_mm(12:20));
finalSigmasX(3) = (sigmaX_mm(3) + sigmaX_mm(5) + sigmaX_mm(14))/3;
finalSigmasY(3) = (sigmaY_mm(3) + sigmaY_mm(5) + sigmaY_mm(14))/3;
finalSigmasX(5) = sigmaX_mm(16);
finalSigmasY(5) = sigmaY_mm(16);
finalSigmasX(2) = 1.31;
finalSigmas = 0.5*(finalSigmasX + finalSigmasY);
figure(3)
plot(Zpos2(theMask), finalSigmasX(theMask), 'bo');
hold on
plot(Zpos2(theMask), finalSigmasY(theMask), 'rx');
plot(Zpos2(theMask), finalSigmas(theMask), 'kd-');
ylabel('Sigma (mm)')
xlabel('Distance (cm)');
...
errorbar(Zpos2(theMask), sigmaX_mm(11+theMask), deltasigmaX_mm(11+theMask),'b.');
errorbar(Zpos2(theMask), sigmaY_mm(11+theMask), deltasigmaY_mm(11+theMask),'m.');
...
errorbar(Zpos2(theMask), 0.2*meanDoses_3mm_corr(11+theMask), 0.2*stdDoses_3mm_corr(11+theMask),'b.');
plot(Zpos2(theMask), meanDoses_3mm_corr(1).*intRelFractions.*Sw_rel(theMask)', 'k:');
