clear all; close all
radNames = {'Rad5b','Rad8','Rad12','Rad14','Rad18'};
allSw_preFilm = [];
alldSw_preFilm = [];
allratios = [];
alldratios = [];
...
for i = 1:numel(radNames)
    ...
    validMask = ~isnan(Sw_preFilm) & ~isnan(dSw_preFilm) & ~isnan(ratios) & ~isnan(dratios);
    allSw_preFilm = [allSw_preFilm, Sw_preFilm(validMask)];
    alldSw_preFilm = [alldSw_preFilm, dSw_preFilm(validMask)];
    allratios = [allratios, ratios(validMask)];
    alldratios = [alldratios, dratios(validMask)];    
    %pause
end
