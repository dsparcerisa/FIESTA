clear all
load('preData_29oct_EBT3unl.mat');
E = myData(:,1);
LET = myData(:,2);
ratio = myData(:,3);
dratio = myData(:,4);
...
grid on
wgt = dratio.^(-2);
F = fit(E(vM), ratio(vM), 'rat11', 'Weights', wgt(vM))
hold on;
Evals = 0.1:0.1:10;
...
figure(2);
hold off
errorbar(LET(vM),ratio(vM),dratio(vM),'b.'); hold on
...
F2 = fit(LET(vM), ratio(vM), ft2, 'Weights', wgt(vM), 'Lower', [0 0])
